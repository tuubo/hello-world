...
XML の代わりに JSON を用いることが、XXE 対策となる。

## 4.15 共有資源やキャッシュに関する問題

変数を共有変数として持つと、意図せず他の人のリクエストの結果が見えてしまう可能性がある。共有する必要のない値はローカル変数として持つことで問題が解消される。

キャッシュが過剰に残ると、個人情報漏洩のリスクがある。アプリ側とキャッシュサーバ側で適切なキャッシュ制御の設定を行う必要がある。

## 4.16 Web API 実装における脆弱性

Web API では JSON 関連の脆弱性が生じやすい

JSON データの読み込み時に、適切なエスケープ処理の実行や関数の使用をしていないと脆弱性の原因となる。

JSON を直接呼び出す場合、MIME タイプが正しく設定されていないとスクリプトが実行され XSS 脆弱性へとつながる危険がある。

### Web API のクロスサイト・リクエストフォージェリ

別のユーザが重要な処理を実行してしまう危険性がある。
...
### jQuery セレクタの動的生成による XSS

$関数に HTML タグを指定すると、DOM 要素を生成する。ここに外部からの入力が混ざると攻撃者が新たな要素を生成できる脆弱性につながる。

### JavaScript の脆弱性対策

- 適切な DOM 操作、記号のエスケープを行う
- eval, setTimeout, Function コンストラクタなどの引数に外部から文字列形式での値を渡さない
- URL のスキームの限定
- jQuery のセレクタを動的生成しない
- 最新のライブラリを用いる
